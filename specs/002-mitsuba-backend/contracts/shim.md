...
| Mitsuba headers compile standalone | Open | None | None | Compile a file including `<mitsuba/render/scene.h>` that instantiates `Properties` and links. **This is the first gate; if it fails, fall back to `pyo3` per `research.md` D3.** |
...
- **Mitsuba aborts rather than throwing:** not recoverable. If this
  happens, the shim cannot be made safe and `pyo3` becomes the path.
