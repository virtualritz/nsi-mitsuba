# Plan: ɴsɪ Scene Recording And Resolution
...
| Arguments copy losslessly | `cargo test -p nsi-record --lib owned` | yes |
| Classification is exhaustive | `cargo test -p nsi-record --test classifier` | yes |
| Resolution is correct | `cargo test -p nsi-record --lib resolve` | yes |
| **Fidelity against 3Delight** | `cargo test -p nsi-record --test stream_roundtrip` | yes |
